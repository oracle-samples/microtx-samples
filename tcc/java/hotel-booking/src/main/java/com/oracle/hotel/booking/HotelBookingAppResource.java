...
*/
package com.oracle.hotel.booking;
...
public class HotelBookingAppResource extends Application {
    ...
    private static final Logger log = Logger.getLogger(HotelBookingAppResource.class.getSimpleName());

    @POST
    @Path("bookings")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response create(@QueryParam("hotelName") @DefaultValue("Hotel-01") String hotelName) throws TccUnknownTransactionException {
        ...
        // Register participant with the TCC transactiona
        try {
            tccClientService.addTccParticipant(bookingUri.toString());
        } catch (TccUnknownTransactionException e) {
            log.severe("Hotel booking failed "+e.getLocalizedMessage());
            e.printStackTrace();
        }
