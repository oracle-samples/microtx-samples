...
*/
package com.oracle.springhotelbooking.controller;


import com.oracle.microtx.tcc.MicroTxTccClient;
import com.oracle.microtx.tcc.annotation.TCC;
import com.oracle.springhotelbooking.entity.Booking;
...
public class HotelBookingResource {
    ...
    private int totalBookingsAllowed = 30;
    Map<String, Booking> bookings = new HashMap<>();

    private static final Logger logger = LoggerFactory.getLogger(HotelBookingResource.class);


    @RequestMapping(value = "bookings", method = RequestMethod.POST)
    public ResponseEntity<?> create(@RequestParam(value="hotelName", required = true ,defaultValue = "Hotel-01") String  hotelName) throws TccUnknownTransactionException {
        ...
        StringBuilder bookingUri = new StringBuilder().append(bookingBaseUrl).append("/").append(hotelBookingId.toString());

        Booking booking = new Booking(bookingUri.toString(), System.currentTimeMillis(), tccClientService.getTimeLimit(),hotelBookingId.toString());
        booking.setBookingMetadata("HOTEL", hotelName);
        bookings.put(hotelBookingId.toString(), booking);
        ...
        try {
            ...
            logger.error("Hotel booking failed :" + e.getLocalizedMessage());
            e.printStackTrace();
        }
