...
rmEndPoint = 'http://localhost:8081'
booking_base_url = 'http://localhost:8081/api/bookings'
...
def doHotelBooking():
    ...
    hotelBookingId = str(uuid.uuid1())

    hotel_name = request.args.get('hotelName')
    if not hotel_name:
        hotel_name = 'Hotel-01'

    bookingUri = booking_base_url + "/" + str(hotelBookingId)
    currBookingID += 1
    booking = models.Booking.Booking(bookingUri, int(time.time() * 1000), tccConfig.timeInMilliSeconds, hotelBookingId, hotel_name)
    global bookings
    bookings[hotelBookingId] = booking
    try:
        tccConfig.addTccParticipant(bookingUri)
    except Exception as e:
        logger.error('Failed with exception [%s]' % e)
        traceback.print_exc()
        return make_response(jsonify({'message': 'Hotel booking failed'}), 500)
...
def doConfirmBooking(bookingId):
    ...
    if checkIfExpired(booking):
        ...
        return make_response(jsonify({'message': 'Hotel Booking expired'}), 500)
    del bookings[bookingId]
...
if __name__ == '__main__':
    logger.info("Hotel booking Service 2: on port=" + str(service_port))
