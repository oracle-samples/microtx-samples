...
const tccConfig: TCCConfig = new TCCConfig("/bookings", flightSvcRouter, HttpMethod.POST, 60);

flightSvcRouter.post('/bookings', asyncHandler(async (req: Request, resp: Response) => {
    doFlightBooking(req, resp); //business logic
}));

flightSvcRouter.put('/bookings/:bookingId', asyncHandler(async (req: Request, resp: Response) => {
    doConfirmBooking(req, resp); //business logic
}));


flightSvcRouter.delete('/bookings/:bookingId', asyncHandler(async (req: Request, resp: Response) => {
    doCancelBooking(req, resp); //business logic
}));

function doFlightBooking(req: Request, resp: Response) {
    console.log(`Flight service booking called with tccId:` + GetTCCId(req.headers));
    ...
    if(curBookingId > totalBookingsAllowed){
        console.log(`Maximum Flight bookings reached`);
        let responseBody : ErrorResponse = {
            message: "Maximum Flight bookings reached"
        }
        return resp.status(500).send(JSON.stringify(responseBody));
    }
    const flightBookingId:string = randomUUID();
    const booking: Booking = {
        bookingUri: `${bookingBaseUrl}/${flightBookingId}`,
        startTime: Date.now(),
        expires: tccConfig.timeInMilliSeconds,
        bookingId: flightBookingId,
        name: req.query.flightNumber as string
    };
    bookings.set(flightBookingId, booking);
    curBookingId++;
    tccConfig.addTccParticipant(booking.bookingUri);
    console.log(`Flight Booking: ${JSON.stringify(booking)}`);
    resp.status(200).type('application/json').send(JSON.stringify(booking));
}

function doConfirmBooking(req: Request, resp: Response) {
    console.log(`Flight service confirm booking called with tccId:` + GetTCCId(req.headers));
    ...
    console.log(`Flight booking [ ${req.params.bookingId} ] confirmed with tcc transaction Id:` , GetTCCId(req.headers));
    resp.status(200).send();

}
function doCancelBooking(req: Request, resp: Response) {
    console.log(`Flight service cancel booking called with tccId:` + GetTCCId(req.headers));
    const booking = bookings.get(req.params.bookingId);
    if (!booking) {
        console.log(`Booking not found, Id: ${req.params.bookingId}`);
        return resp.status(404).send();
