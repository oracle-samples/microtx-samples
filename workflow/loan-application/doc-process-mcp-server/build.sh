...
JAR_FILE="prebuilt/doc-process-mcp-server-0.0.1-SNAPSHOT.jar"
...
if [[ "$REBUILD" == "true" ]]; then
    ...
    cp -f target/doc-process-mcp-server-0.0.1-SNAPSHOT.jar "$JAR_FILE"
...
if [[ "$BUILD_USING_MINIKUBE" == "true" ]]; then
    ...
    minikube image build -t doc-process-mcp-server:latest .
    echo "Build and Minikube image creation successful."
else
    # Build the Docker image
    $DOCKER build -t doc-process-mcp-server:latest .
