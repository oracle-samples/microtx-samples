...
public class PropagationNeverResource {
    ...
    public Response transferTransactionalNeverOC(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawNeverOC_AnnotatedRequiresNew(transferDetails);
        Response depositResponse = transferHelpers.depositNeverOC_AnnotatedRequiresNew(transferDetails);
        Response emailResponse = notificationHelpers.emailNotifyNeverOC_AnnotatedNever(transferDetails);
    ...
    public Response transferTransactionalNeverIC(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawNeverIC_AnnotatedMandatory(transferDetails);
        Response depositResponse = transferHelpers.depositNeverIC_AnnotatedMandatory(transferDetails);
        Response emailResponse = notificationHelpers.emailNotifyNeverIC_AnnotatedNever(transferDetails);
        if (withdrawResponse.getStatus() == Response.Status.OK.getStatusCode() && depositResponse.getStatus() == Response.Status.OK.getStatusCode()
                && emailResponse.getStatus() == Response.Status.OK.getStatusCode()) {
            return Response.status(Response.Status.OK.getStatusCode(), "Transfer completed successfully").build();
        }
