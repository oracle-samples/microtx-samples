...
public class PropagationNotSupportedResource {
    ...
    public Response transferTransactionalNotSupportsOC(Transfer transferDetails) throws SystemException, HeuristicRollbackException, HeuristicMixedException, RollbackException {
        ...
        Response withdrawResponse = transferHelpers.withdrawNotSupportedOC_AnnotatedRequiresNew(transferDetails);
        Response depositResponse = transferHelpers.depositNotSupportedOC_AnnotatedRequiresNew(transferDetails);
        Response emailResponse = notificationHelpers.emailNotifyNotSupportedOC_AnnotatedNotSupported(transferDetails);
    ...
    //recheck failure once, it can be expected
    @POST
    ...
    public Response transferTransactionalNotSupportsIC(Transfer transferDetails) throws SystemException, HeuristicRollbackException, HeuristicMixedException, RollbackException {
        ...
        Response withdrawResponse = transferHelpers.withdrawNotSupportedIC_AnnotatedMandatory(transferDetails);
        Response depositResponse = transferHelpers.depositNotSupportedIC_AnnotatedSupports(transferDetails);
        Response emailResponse = notificationHelpers.emailNotifyNotSupportedIC_AnnotatedNotSupported(transferDetails);
        if (withdrawResponse.getStatus() == Response.Status.OK.getStatusCode() && depositResponse.getStatus() == Response.Status.OK.getStatusCode()
                && emailResponse.getStatus() == Response.Status.OK.getStatusCode()) {
            return Response.status(Response.Status.OK.getStatusCode(), "Transfer completed successfully").build();
        }
