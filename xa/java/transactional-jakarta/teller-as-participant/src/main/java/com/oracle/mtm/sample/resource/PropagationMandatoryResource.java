...
public class PropagationMandatoryResource {
    ...
    public Response transferTransactionalMandatoryTxnOC(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawMandatoryOC_AnnotatedRequiresNew(transferDetails);
        Response depositResponse = transferHelpers.depositMandatoryOC_AnnotatedMandatory(transferDetails);
    ...
    // this is failing : TrmTransactionalInterceptor exception: class java.lang.IllegalStateException : There is already a transaction associated with the thread
    @POST
    @Path("MANDATORY/insideContext")
    @Consumes(MediaType.APPLICATION_JSON)
    @Transactional(Transactional.TxType.REQUIRED)
    public Response transferTransactionalMandatoryTxnIC(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawMandatoryIC_AnnotatedRequiresNew(transferDetails);
        Response depositResponse = transferHelpers.depositMandatoryIC_AnnotatedMandatory(transferDetails);
        if (withdrawResponse.getStatus() == Response.Status.OK.getStatusCode() && depositResponse.getStatus() == Response.Status.OK.getStatusCode()) {
            return Response.status(Response.Status.OK.getStatusCode(), "Transfer completed successfully").build();
        }
    ...
    @POST
    @Path("MANDATORY/insideContext/REQUIRED")
    @Consumes(MediaType.APPLICATION_JSON)
    @Transactional(Transactional.TxType.REQUIRED)
    public Response transferTransactionalRequiredNMandatoryTxnIC(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawMandatoryRequiredIC_AnnotatedRequired(transferDetails);
        Response depositResponse = transferHelpers.depositMandatoryIC_AnnotatedMandatory(transferDetails);
        if (withdrawResponse.getStatus() == Response.Status.OK.getStatusCode() && depositResponse.getStatus() == Response.Status.OK.getStatusCode()) {
            return Response.status(Response.Status.OK.getStatusCode(), "Transfer completed successfully").build();
        }
    ...
    public Response transferTransactionalMandatoryTxnICNested(Transfer transferDetails) {
        Response withdrawResponse = transferHelpers.withdrawMandatoryNestedIC_AnnotatedRequired(transferDetails);
