...
import com.oracle.mtm.sample.data.EmailService;
...
public class AccountsResource {
    ...
    @POST
    @Path("{accountId}/emailNotify")
    public Response emailNotify(@PathParam("accountId") String accountId, @QueryParam("amount") double amount) {
        try {
            this.emailService.updateEmail(accountId, "DEBITED", amount);
            logger.info( amount + " email details updated in account: " + accountId);
            return Response.status(Response.Status.OK.getStatusCode(), " email details updated in account: " + accountId)
                    .build();
    ...
    @GET
    @Path("{accountId}/emailNotification")
    public Response emailNotification(@PathParam("accountId") String accountId) {
        try {
            return Response.status(Response.Status.OK.getStatusCode())
                    .entity(this.emailService.emailDetails(accountId))
                    .build();
        } catch (Exception e) {
            logger.error( e.getLocalizedMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
    ...
    public Response getMemberList() {
        List<String> memberList = Arrays.asList("apple", "banana");
        return Response
                .status(200)
