...
public class AccountsResource {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Table already exists ")
    })
    @RequestMapping(value = "/populateData", method = RequestMethod.GET)
    public ResponseEntity<?> populateAccountsData() {
        try {
            this.accountService.populateAccounts();
            return ResponseEntity.ok("{ \"message\" : \"Accounts data populated successful\" }");
        } catch (SQLException e) {
            LOG.error(e.getLocalizedMessage());
            return ResponseEntity.internalServerError().body( String.format("{ \"message\" : \" %s \" }",e.getLocalizedMessage()));
        }
    }
