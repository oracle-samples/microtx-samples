...
public class Configuration {
    ...
    private void initialiseXaDataSource() {
        try {
            EmbeddedXADataSource xaDataSource ;
            xaDataSource = new EmbeddedXADataSource();
            //For persistent file-based data storage, use the JDBC connection string "mydb" instead of "memory:mydb".
            xaDataSource.setDatabaseName("memory:mydb"); // Set your database name
            xaDataSource.setCreateDatabase("create");
            TrmConfig.initXaDataSource(xaDataSource);
            PopulateData.populateData(xaDataSource);
        } catch (Exception e) {
