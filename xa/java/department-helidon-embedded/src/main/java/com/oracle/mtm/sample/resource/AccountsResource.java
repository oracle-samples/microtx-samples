...
public class AccountsResource {
    ...
    /**
     * API to load dummy data in database
     */
    @APIResponses(value = {
            @APIResponse(responseCode = "200", description = "Data Loaded Successful",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON, schema = @Schema(ref = "Account"))),
            @APIResponse(responseCode = "404", description = "No account found for the provided account Identity"),
            @APIResponse(responseCode = "500", description = "Internal Server Error")
    })
    ...
    @Path("/populateData")
    @Produces(MediaType.APPLICATION_JSON)
    public Response populateAccountsData() {
        try {
            this.accountService.populateAccounts();
            return Response.status(Response.Status.OK).entity("{ \"message\" : \"Accounts data populated successful\" }").build();
        } catch (SQLException e) {
            logger.error(e.getLocalizedMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(String.format("{ \"message\" : \" %s \" }",e.getLocalizedMessage())).build();
        }
    }
