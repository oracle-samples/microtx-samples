...
public class PropagationRequiredResource {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(PropagationRequiredResource.class);
    ...
    // If @Transactional annotation is removed. App can explicitly invoke MicroTx txn manager using MicroTxUserTransactionService
    //@Autowired
    //MicroTxUserTransactionService microTxUserTransactionService;

    @RequestMapping(value = "REQUIRED/outsideContext", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredOC(@RequestBody Transfer transferDetails,
    ...
    @RequestMapping(value = "REQUIRED/insideContext", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredIC(@RequestBody Transfer transferDetails){
        ResponseEntity<?> withdrawResponse = transferHelpers.withdrawRequiredIC_AnnotatedRequired(transferDetails);
        ResponseEntity<?> depositResponse = transferHelpers.depositRequiredIC_AnnotatedRequired(transferDetails);
    ...
    @RequestMapping(value = "REQUIRED/nested/insideContext", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredNestedTxn(@RequestBody Transfer transferDetails) throws CustomCheckedException1{
        ResponseEntity<?> withdrawResponse = transferHelpers.withdrawRequiredNestedIC_AnnotatedRequired(transferDetails);
    ...
    @RequestMapping(value = "REQUIRED/insideContext/automic", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredICAutomic(@RequestBody Transfer transferDetails){
        ResponseEntity<?> withdrawResponse = transferHelpers.withdrawRequiredIC_AnnotatedRequired(transferDetails);
    ...
    @RequestMapping(value = "REQUIRED/outsideContext/rollback", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredOCRollback(@RequestBody Transfer transferDetails){
        ResponseEntity<?> withdrawResponse = transferHelpers.withdraw_NoAnnotation(transferDetails);
        ResponseEntity<?> depositResponse = transferHelpers.deposit_NoAnnotation_Exception(transferDetails);
        if (withdrawResponse.getStatusCode().is2xxSuccessful() && depositResponse.getStatusCode().is2xxSuccessful()){
            return ResponseEntity.ok("Transfer completed successfully");
        }
        throw new RuntimeException("Transfer failed ");
    ...
    @RequestMapping(value = "REQUIRED/outsideContext/nestedtxn/rollback", method = RequestMethod.POST)
    @Transactional(propagation = Propagation.REQUIRED)
    public ResponseEntity<?> transferTransactionalRequiredOCNestedTxnRollback(@RequestBody Transfer transferDetails){
        ResponseEntity<?> withdrawResponse = transferHelpers.withdrawRequiredIC_AnnotatedRequired(transferDetails);
        ResponseEntity<?> depositResponse = transferHelpers.depositRequiresNewIC_AnnotatedRequiresNew_Exception(transferDetails);
        if (withdrawResponse.getStatusCode().is2xxSuccessful() && depositResponse.getStatusCode().is2xxSuccessful()){
            return ResponseEntity.ok("Transfer completed successfully");
        }
        throw new RuntimeException("Transfer failed ");
