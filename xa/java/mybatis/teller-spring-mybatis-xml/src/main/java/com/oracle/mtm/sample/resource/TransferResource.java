...
public class TransferResource {
    ...
    public ResponseEntity<?> transfer(@RequestBody Transfer transferDetails, @RequestParam(value = "doRollback", defaultValue = "false") boolean doRollback) {
        ...
        try {
            ...
            if (!withdrawHttpStatus.equals(HttpStatus.OK)) {
                LOG.error("Withdraw failed: {} Reason: {}", transferDetails.toString(), withdrawResponse.getBody());
                throw new RuntimeException("Withdraw failed. Rolling back transaction");
            }
            // Fee processing
            ...
            if (feeDeposited) {
                ...
                throw new RuntimeException("Fee Deposit failed. Rolling back transaction");
            }
            // Deposit processing
            ...
            if (!depositHttpStatus.equals(HttpStatus.OK)) {
                LOG.error("Deposit failed: {} Reason: {}", transferDetails.toString(), depositResponse.getBody());
                throw new RuntimeException("Deposit failed. Rolling back transaction");
            }
            if(doRollback){
                // If user want to rollback explicitly
            ...
            e.printStackTrace();
            throw new RuntimeException("Failed to complete the transaction due to TransactionException.", e);
        } catch (Exception e){
            e.printStackTrace();
            throw new RuntimeException("Failed to complete the transaction due to Exception.", e);
        }
    }
