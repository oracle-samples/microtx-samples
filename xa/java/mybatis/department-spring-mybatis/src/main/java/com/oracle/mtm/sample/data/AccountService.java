...
public class AccountService implements IAccountService {
    ...
    @Autowired
    @Qualifier("microTxMyBatisSqlConnection")
    @Lazy
    private MicroTxSqlSession sqlSession;
    ...
    public Account accountDetails(String accountId) {
        ...
        try (SqlSession session = sqlSessionFactory.openSession()) {
            AccountMapperNonXA accountMapper = session.getMapper(AccountMapperNonXA.class);
            account = accountMapper.findByAccountId(accountId);
        }
        return account;
    }
    ...
    public boolean withdraw(String accountId, double amount) {
        try(MicroTxSqlSession session = (MicroTxSqlSession) sqlSession) {
            AccountMapperXA accountMapper = session.getMapper(AccountMapperXA.class);
            int rowsAffected = accountMapper.withdrawAccountAmount(accountId, amount);
    ...
    public boolean deposit(String accountId, double amount) {
        try(MicroTxSqlSession session = (MicroTxSqlSession) sqlSession) {
            AccountMapperXA accountMapper = session.getMapper(AccountMapperXA.class);
            int rowsAffected = accountMapper.depositAccountAmount(accountId, amount);
            return rowsAffected > 0;
        }
    }
