    ...
    public Response withdraw(@PathParam("accountId") String accountId, @QueryParam("amount") double amount) {
        ...
        try {
            ...
            if(this.accountService.withdraw(accountId, amount)) {
                ...
                if(this.accountService.IncreaseCreditPoint(accountId,1))
                {
                    logger.info(amount + " Credit Point added to account: " + accountId);
                }
                else {
                    logger.info(amount + " Credit Point failed to added for account: " + accountId);
                }
