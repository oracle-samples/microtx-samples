...
import com.oracle.mtm.sample.data.AccountService;
...
public class AccountsResource {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Credit Resource Manager Account Details",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(ref = "Account"))),
            @ApiResponse(responseCode = "404", description = "No account found for the provided credit account Identity"),
            @ApiResponse(responseCode = "500", description = "Internal Server Error")
    })
    @RequestMapping(value = "/credit/{accountId}", method = RequestMethod.GET)
    public ResponseEntity<?> getCreditAccountDetails(@PathVariable("accountId") String accountId) {
        try {
            Account account = this.accountService.creditAccountDetails(accountId);
            if(account == null) {
                LOG.error("Credit Account not found: " + accountId);
                ResponseEntity.status(HttpStatus.NOT_FOUND).body("No account found for the provided credit account Identity");
            }
            return ResponseEntity.ok(account);
        } catch (SQLException e) {
            LOG.error(e.getLocalizedMessage());
    ...
    public ResponseEntity<?>  withdraw(@PathVariable("accountId") String accountId, @RequestParam("amount") double amount) {
        ...
        try {
            ...
            if(this.accountService.withdraw(accountId, amount)) {
                ...
                {
                    LOG.info(creditPointAmount +" Credit Point added to account: " + accountId);
                }
                else {
                    LOG.info(creditPointAmount + " Credit Point failed to added for account: " + accountId);
                }
