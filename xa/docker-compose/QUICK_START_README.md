...
1. Load the Transaction Manager for Microservices image in the local Docker repository. The Transaction Manager for Microservices image is located at `installation_directory/otmm-24.2.1/otmm/image/tmm-24.2.1.tgz`.

   ```bash
   cd installation_directory/otmm-24.2.1/otmm
   docker load < image/tmm-24.2.1.tgz
   ```
   ...
   ```bash
   cd installation_directory/otmm-24.2.1/otmm
   docker load -i image/tmm-24.2.1.tgz
   ```
...
2. Create a tag for the above image to push it into the registry that you started for swarm.
   ...
   tmm                                                            24.2.1      bcc91ec952a6   4 days ago      610MB
   ```

   b. Use the following command to create a new tag.

   ```bash
   docker tag tmm:24.2.1 127.0.0.1:5000/tmm
   ```
...
The XA sample application is available in the installation bundle in the `installation_directory/otmm-24.2.1/samples/xa/java` folder.
...
2. Unzip the wallet file.
   ...
   2. Copy the wallet files to the following folders that contain the source code for the participant applications. Perform this step for both ATP instance wallets for the two participant applications.
      - `installation_directory/otmm-24.2.1/samples/xa/java/department-helidon/Database_Wallet/ `
      - `installation_directory/otmm-24.2.1/samples/xa/java/department-spring/Database_Wallet/ `
...
1. Run the following SQL file using SQL Developer or SQL Plus for the first database:
   ...
   ```bash
   < installation_directory/otmm-24.2.1/samples/xa/java/department-helidon/department.sql
   ```
...
2. Run the following SQL file using SQL Developer or SQL Plus for the second database:
   ...
   ```bash
   < installation_directory/otmm-24.2.1/samples/xa/java/department-spring/department.sql
   ```
...
The XA sample application is available in the installation bundle in the `installation_directory/otmm-24.2.1/samples/xa/java` folder. This folder contains the code for three microservices, YAML file, and Helm charts.
...
1. Run the following commands to build the Docker image for the teller application. Then, push the image to the docker registry after the build is completed
   ...
   ```bash
   cd installation_directory/otmm-24.2.1/samples/xa/java/teller
...
2. Run the following commands to build the Docker image for the Department 1 application. Then, push the image to the docker registry after the build is completed.
   ...
   ```bash
   cd installation_directory/otmm-24.2.1/samples/xa/java/department-helidon
...
3. Run the following commands to build the Docker image for the Department 2 application. Then, push the image to the docker registry after the build is completed.
   ...
   ```bash
   cd installation_directory/otmm-24.2.1/samples/xa/java/department-spring
