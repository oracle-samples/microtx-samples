 ...
 */
@RestController
@RequestMapping("sell-transaction-sync")
@Slf4j
public class SellStockEventListenerResource {
    ...
    public ResponseEntity<?> afterCompletion(@PathVariable("gtrid") String gtrid, @PathVariable("status") String status) {
        try {
            SellStock sellStock = sellStockTransactionContext.get(gtrid);
            StocksEntry stocksEntry = stockService.getStock(sellStock.getStockSymbol());
            if (status.equals("STATUS_COMMITTED")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(sellStock.getUserAccountId())
                        .stockOperation(StockOperation.SELL)
                        .stockSymbol(sellStock.getStockSymbol())
                        .stockUnits(sellStock.getStockUnits())
                        .description(String.format("User successfully sold %s stocks.",stocksEntry.getCompanyName()))
                        .build());
            } else if (status.equals("STATUS_ROLLEDBACK")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(sellStock.getUserAccountId())
                        .stockOperation(StockOperation.SELL_FAILED)
                        .stockSymbol(sellStock.getStockSymbol())
                        .stockUnits(sellStock.getStockUnits())
                        .description(String.format("Selling %s stocks failed.",stocksEntry.getCompanyName()))
                        .build());
            }
            sellStockTransactionContext.remove(gtrid);
