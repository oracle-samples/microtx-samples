...
import com.oracle.tmm.stockbroker.domain.response.CommonResponse;
...
@OpenAPIDefinition(info = @Info(title = "Stock Transactions history endpoint", version = "1.0"))
@Slf4j
public class HistoryResource {
    ...
    @RequestMapping(value = "logTransaction", method = RequestMethod.POST, produces = {MediaType.APPLICATION_JSON_VALUE}, consumes = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> logTxn(@RequestBody TransactionHistoryEntry transactionHistory) throws Exception {

        if (historyService.logTransaction(transactionHistory)) {
            return ResponseEntity.ok()
                    .body(CommonResponse.builder()
                            .message("Created Transaction history successfully.")
                            .build());
        }
        return ResponseEntity.internalServerError()
                .body(ErrorResponse.builder()
                        .errorMessage("Transaction history creation failed.")
                        .build());
    }
    ...
    public ResponseEntity<?> getTxnHistory(@RequestParam(value = "accountId", required = false) Integer accountId) {
        ...
        try {
            ...
            transactionHistories.setMessage("Transaction history retrieved successfully.");
            return ResponseEntity.ok()
                    .body(transactionHistories);
        } catch (SQLException e) {
            return ResponseEntity.internalServerError()
                    .body(ErrorResponse.builder()
                            .errorMessage("Retrieving Transaction history failed.")
                            .build());
        }
    }
