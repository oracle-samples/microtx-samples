 ...
 */
@RestController
@RequestMapping("bank-transaction-sync")
@Slf4j
public class TransferEventListenerResource {
    ...
    public ResponseEntity<?> afterCompletion(@PathVariable("gtrid") String gtrid, @PathVariable("status") String status) {
        try {
            TransferRequest transferRequest = bankTransactionContext.get(gtrid);
            ...
            if (status.equals("STATUS_COMMITTED")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(transferRequest.getFromAccountId()).branchId(sourceAccountDetails.getBranchId())
                        .transactionType(TransactionType.TRANSFER)
                        .description(String.format("Transfer successful from account %s to %s. Remarks: %s.", transferRequest.getFromAccountId(), transferRequest.getToAccountId(), transferRequest.getRemarks()))
                        .amount(transferRequest.getAmount()).balance(bankTransactionService.balanceEnquiry(transferRequest.getFromAccountId()).getAccountBalance())
                        .build());
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(transferRequest.getToAccountId()).branchId(sourceAccountDetails.getBranchId())
                        .transactionType(TransactionType.DEPOSIT)
                        .description(String.format("Deposit successful to account %s. Remarks: %s.", transferRequest.getToAccountId(), transferRequest.getRemarks()))
                        .amount(transferRequest.getAmount()).balance(bankTransactionService.balanceEnquiry(transferRequest.getToAccountId()).getAccountBalance())
                        .build());
            } else if (status.equals("STATUS_ROLLEDBACK")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(transferRequest.getFromAccountId()).branchId(sourceAccountDetails.getBranchId())
                        .transactionType(TransactionType.TRANSFER_FAILED)
                        .description(String.format("Transfer Failed from account %s to %s.", transferRequest.getFromAccountId(), transferRequest.getToAccountId()))
                        .amount(transferRequest.getAmount()).balance(bankTransactionService.balanceEnquiry(transferRequest.getFromAccountId()).getAccountBalance())
                        .build());
            }
            bankTransactionContext.remove(gtrid);
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            e.printStackTrace();
            log.error("Exception while logging the transaction history in transaction callback events. {}", e.getMessage());
