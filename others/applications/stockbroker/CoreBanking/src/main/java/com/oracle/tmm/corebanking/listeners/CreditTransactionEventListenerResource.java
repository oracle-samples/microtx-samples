 ...
 */
@RestController
@RequestMapping("credit-transaction-sync")
@Slf4j
public class CreditTransactionEventListenerResource {
    ...
    public ResponseEntity<?> afterCompletion(@PathVariable("gtrid") String gtrid, @PathVariable("status") String status) {
        try {
            CreditRequest creditRequest = creditTransactionContext.get(gtrid);
            AccountBranchDetails sourceAccountDetails = branchService.getBranchDetailsByUserAccount(creditRequest.getAccountId());
            if (status.equals("STATUS_COMMITTED")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(creditRequest.getAccountId()).branchId(sourceAccountDetails.getBranchId())
                        .transactionType(TransactionType.CREDIT)
                        .description(String.format("Amount credited for selling stocks to account %s.", creditRequest.getAccountId()))
                        .amount(creditRequest.getAmount())
                        .balance(bankTransactionService.balanceEnquiry(creditRequest.getAccountId()).getAccountBalance())
                        .build());
            } else if (status.equals("STATUS_ROLLEDBACK")) {
                historyService.logTransaction(TransactionHistoryEntry.builder().accountId(creditRequest.getAccountId()).branchId(sourceAccountDetails.getBranchId())
                        .transactionType(TransactionType.CREDIT_FAILED)
                        .description(String.format("Crediting amount for selling stocks failed on account %s.", creditRequest.getAccountId()))
                        .amount(creditRequest.getAmount())
                        .balance(bankTransactionService.balanceEnquiry(creditRequest.getAccountId()).getAccountBalance())
                        .build());
            }
            creditTransactionContext.remove(gtrid);
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            e.printStackTrace();
