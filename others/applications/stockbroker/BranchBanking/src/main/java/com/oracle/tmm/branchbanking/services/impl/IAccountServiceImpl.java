...
public class IAccountServiceImpl implements IAccountService {
    ...
    @Override
    public boolean openAccount(AccountEntry accountEntry) throws SQLException {
        String query = "INSERT INTO SAVINGS_ACCOUNT (ACCOUNT_ID, BRANCH_ID, BALANCE) " +
                "VALUES (?, ?, ?)";
        try(PreparedStatement statement = connection.prepareStatement(query);) {
            statement.setInt(1, accountEntry.getAccountNumber());
            statement.setInt(2, accountEntry.getBranchId());
            statement.setDouble(3, accountEntry.getBalance());
    ...
    public boolean closeAccount(Integer accountId) throws SQLException {
        String query = "DELETE FROM SAVINGS_ACCOUNT " +
                "WHERE ACCOUNT_ID=?";
        try(PreparedStatement statement = connection.prepareStatement(query);) {
