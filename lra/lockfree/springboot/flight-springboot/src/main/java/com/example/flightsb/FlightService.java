...
public class FlightService {
    ...
    public Booking book(String bookingId, String flight, Connection connection) {
        Booking booking = new Booking(bookingId, flight, "Flight", null);

        try (Statement s = connection.createStatement()) {
            s.executeUpdate("UPDATE flights SET seats = seats - 1 WHERE id = 1");
            // Local commit for the decrement
            connection.commit();
        } catch (SQLException ex) {
            booking.setStatus(Booking.BookingStatus.FAILED);
            LOG.error("Error while reserving flight", ex);
        }
